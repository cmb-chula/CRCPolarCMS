...
!wget --content-disposition https://figshare.com/ndownloader/files/32991419
...
def directional_gsea(term , target_genes, fdr = 0.05, linewidth = 4, display = True):
    ...
    if term in gsea_results[0.06283185307179529].res2d.index:
        ...
        for angle in directional_pearson_correlation.columns.values:
            temp = pd.concat([pd.DataFrame(directional_pearson_correlation.index.values, index = directional_pearson_correlation.index), \
                              directional_pearson_correlation[angle]], axis = 1)
